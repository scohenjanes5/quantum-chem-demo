...
def getArgs():
    ...
    acid_charicteristics.add_argument(
        "-Ka",
        type=float,
        nargs="*",
        help="Equilibrium constants. Up to 3 allowed.",
        default=[1e-4, 0, 0],
    )
    acid_charicteristics.add_argument(
        "-pKa", type=float, nargs="*", help="pKa values. Up to 3 allowed.", default=[]
    )
    parser.add_argument(
        "-Ca", type=float, help="Concentration of the acid", default=0.1
    )
    parser.add_argument(
        "-Cb", type=float, help="Concentration of the base", default=0.1
    )
    parser.add_argument(
        "-Va", type=float, help="Volume of the acid stock solution", default=0.01
    )
    parser.add_argument(
        "-Vw", type=float, help="Volume of water used to dilute acid.", default=0
    )
    parser.add_argument("-Kw", type=float, help="Ion product of water", default=1e-14)
    parser.add_argument(
        "--export",
        help="Export the polynomial data to a file",
        action="store_true",
        default=False,
    )
    parser.add_argument(
        "--approx",
        help="Approximate pH for monoprotic acids",
        action="store_true",
        default=False,
    )
    parser.add_argument(
        "--fast",
        help="Use a faster method to calculate pH",
        action="store_true",
        default=False,
    )
    ...
    if args.approx == True and (
        len(args.pKa) > num_Ka_supported_for_approx
        or np.count_nonzero(args.Ka) > num_Ka_supported_for_approx
    ):
...
def pH(args, Vb=0, name="params.txt"):
    ...
    solution = sp.solve(
        x_0 + x_1 * x + x_2 * x**2 + x_3 * x**3 + x_4 * x**4 + x_5 * x**5, x
    )
...
def main():
    ...
    if not args.fast:
        ...
        while last_pH < 12 or derivative > derivative_threshold:
            ...
            print(
                f"Volume: {last_volume:.2f}L, pH: {last_pH:.2f}, derivative: {derivative:.2f}"
            )
